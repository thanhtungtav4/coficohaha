...
if ( ! function_exists( 'presscore_post_navigation' ) ) :
	...
	function presscore_post_navigation( $args = array() ) {
		...
		// previos link
		$previous_post_link = get_previous_post_link( '%link', $prev_post_text );
...
if ( ! function_exists( 'presscore_get_fullwidth_slider_two' ) ) :
	...
	function presscore_get_fullwidth_slider_two( $attachments_data, $options = array() ) {
		...
		$slider_title = $category[0]->name.' TƯƠNG TỰ';
...
if ( ! function_exists( 'presscore_display_related_posts_new' ) ) :

	/**
	 * Display Newer related posts.
	 *
	 */
	function presscore_display_related_posts_new() {
		...
		if ( $terms && !is_wp_error($terms) ) {
			...
			add_filter( 'posts_where', 'after_current_post_new' );
			...
			$head_title = esc_html( __( 'Newer post', LANGUAGE_ZONE ) );
			...
			if ( $posts_list ) {
				...
				// add title
				if ( $head_title ) {
					$html = '<h2 class="entry-title related-title">' . $head_title . '</h2><div class="gap-10"></div>' . $html;
				}

				$html = $html . '<div class="gap-10"></div>';

			}
			remove_filter( 'posts_where', 'after_current_post_new' );
		}
		// echo (string) apply_filters( 'presscore_display_related_posts_new', $html );

		echo '<h2 class="entry-title related-title">' . $head_title . '</h2><div class="gap-10"></div>';

		echo do_shortcode('[ajax_load_more post_type="post" category="'.get_cat_slug( $terms[0]).'" meta_value="'.get_the_date('Y-m-d H:i:s',$post->ID).'" meta_compare="after" scroll="false" button_label="'.__( 'Load more', LANGUAGE_ZONE ).'"]');
			}
...
if ( ! function_exists( 'presscore_display_related_posts' ) ) :
	...
	function presscore_display_related_posts() {
		...
		if ( $terms && !is_wp_error($terms) ) {
			...
			$head_title = esc_html( __( 'Older post', LANGUAGE_ZONE ) );
			...
			if ( $posts_list ) {
				...
				$html = '<div class="gap-30"></div>' . $html . '<div class="gap-10"></div>';
			}
		...
		echo do_shortcode('[ajax_load_more post_type="post" category="'.get_cat_slug( $terms[0]).'" meta_value="'.get_the_date('Y-m-d H:i:s',$post->ID).'" meta_compare="before" scroll="false" button_label="'.__( 'Load more', LANGUAGE_ZONE ).'"]');
	}
