	...
	class lshowcase_ordering {
		...
		public static function admin_head() {
			...
			$screen->add_help_tab(array(
				...
				'content' => '<p>' . __( 'To reposition an item, simply drag and drop the row by "clicking and holding" it anywhere (outside of the links and form controls) and moving it to its new position.', 'lshowcase' ) . '</p>',
			));
		}
		...
		public static function sort_by_order_link( $views ) {
			...
			if(get_query_var('orderby') != 'menu_order') {
				...
				$views['byorder'] = sprintf('<a href="%s" class="%s">%s</a>', $query_string, $class, __("Sort by Order", 'lshowcase'));
					

			} else {

				$views['byorder'] = __("Drag & Drop Ordering Activated", 'lshowcase');
